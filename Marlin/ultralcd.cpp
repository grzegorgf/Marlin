...
char lcd_status_message[4 * (LCD_WIDTH) + 1] = WELCOME_MSG; // worst case is kana with up to 3*LCD_WIDTH+1
...
void lcd_setstatus(const char * const message, const bool persist) {
  ...
  strncpy(lcd_status_message, message, 4 * (LCD_WIDTH));
...
void lcd_setstatusPGM(const char * const message, int8_t level) {
  ...
  strncpy_P(lcd_status_message, message, 4 * (LCD_WIDTH));
...
void lcd_status_printf_P(const uint8_t level, const char * const fmt, ...) {
  ...
  va_list args;
  va_start(args, fmt);
  vsnprintf_P(lcd_status_message, 4 * (LCD_WIDTH), fmt, args);
  va_end(args);
  lcd_finishstatus(level > 0);
}
