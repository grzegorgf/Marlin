...
#if defined(E_MIN_PIN) || defined(E_MAX_PIN) || defined(F_MIN_PIN) || defined(F_MAX_PIN)
  volatile uint16_t
#else
  volatile char
#endif
...
#if ENABLED(Z_DUAL_ENDSTOPS) || defined(E_MIN_PIN) || defined(E_MAX_PIN) || defined(F_MIN_PIN) || defined(F_MAX_PIN)
  uint16_t
#else
  byte
#endif
...
void Endstops::init() {
  ...
  //F min/max
    #ifdef F_MIN_PIN
      #if ENABLED(ENDSTOPPULLUP_FMIN)
        SET_INPUT_PULLUP(F_MIN_PIN);
      #else
        // SET_INPUT(F_MIN_PIN);
        SET_INPUT_PULLUP(F_MIN_PIN);
      #endif
    ...
    #ifdef F_MAX_PIN
      #if ENABLED(ENDSTOPPULLUP_FMAX)
        SET_INPUT_PULLUP(F_MAX_PIN);
      #else
        // SET_INPUT(F_MAX_PIN);
        SET_INPUT_PULLUP(F_MAX_PIN);
      #endif
...
void Endstops::report_state() {
  if (endstop_hit_bits) {
      ...
      char chrX = ' ', chrY = ' ', chrZ = ' ', chrP = ' ', chrE = ' ', chrF = ' ';
    ...
    #endif
    #if defined(F_MIN_PIN) || defined(F_MAX_PIN)
      #define ENDSTOP_HIT_TEST_F() _ENDSTOP_HIT_TEST(F,'F')
    #endif
    ...
    #if ENABLED(ULTRA_LCD)
      //lcd_status_printf_P(0, PSTR(MSG_LCD_ENDSTOPS " %c %c %c %c %c %c"), chrX, chrY, chrZ, chrP, chrE, chrF);
    #endif
...
void Endstops::M119() {
  ...
  //E min/max
    ...
    #endif
  //F min/max
    #if defined(F_MIN_PIN)
      SERIAL_PROTOCOLLNPAIR("f_min: ", ((READ(F_MIN_PIN)^F_MIN_ENDSTOP_INVERTING) ? MSG_ENDSTOP_HIT : MSG_ENDSTOP_OPEN));
    #endif
    #if defined(F_MAX_PIN)
      SERIAL_PROTOCOLLNPAIR("f_max: ", ((READ(F_MAX_PIN)^F_MAX_ENDSTOP_INVERTING) ? MSG_ENDSTOP_HIT : MSG_ENDSTOP_OPEN));
    #endif
...
void Endstops::update() {
  ...
  //F min/max
    if (stepper.current_block->steps[F_AXIS] > 0) {
      if (stepper.motor_direction(F_AXIS)) { // -direction
        #if defined(F_MIN_PIN)
            UPDATE_ENDSTOP(F, MIN);
        #endif
        }
      else { // +direction
        #if defined(F_MAX_PIN)
            UPDATE_ENDSTOP(F, MAX);
        #endif
      }
    }
